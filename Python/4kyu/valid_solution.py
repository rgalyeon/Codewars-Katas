...
def valid_solutions(board):
    for i in range(9):
        col = [board[i][j] for j in range(9)]
        row = [board[j][i] for j in range(9)]
        if sorted(col) != [1, 2, 3, 4, 5, 6, 7, 8, 9] or sorted(row) != [1, 2, 3, 4, 5, 6, 7, 8, 9]:
            return False
    for i in range(3):
        for j in range(3):
            section = [board[i * 3 + k][j * 3 + l] for k in range(3) for l in range(3)]
            if sorted(section) != [1, 2, 3, 4, 5, 6, 7, 8, 9]:
                return False
    return True
